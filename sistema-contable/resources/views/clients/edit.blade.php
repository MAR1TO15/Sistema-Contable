...
<body class="bg-gray-50 dark:bg-gray-900">
    <div class="mx-auto max-w-md px-4 py-8">
        <h1 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Editar cliente</h1>
        ...
        <form method="POST" action="{{ route('clients.update', $client) }}" class="space-y-5">
            ...
            <div>
                ...
                <input id="tax_id" name="tax_id" type="text" value="{{ old('tax_id', $client->tax_id) }}"
                    class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            </div>

            <div>
                <span class="mb-1 block text-sm font-medium text-gray-900 dark:text-white">Contadores asignados</span>
                @forelse ($accountants as $accountant)
                    <label class="mb-1 flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        <input
                            type="checkbox"
                            name="accountant_ids[]"
                            value="{{ $accountant->id }}"
                            @checked(in_array($accountant->id, old('accountant_ids', $assignedIds)))
                            class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        >
                        {{ $accountant->name }}
                    </label>
                @empty
                    <p class="text-sm text-gray-500">Tu firma todavía no tiene contadores registrados.</p>
                @endforelse
            </div>
